...
var server = myHTTPserver
	.use('/add/:a/:b/c/:d/e.html', function(rq,rs,n) {console.log('USE Func1');rs.send("hello");})
	.use('/add/a/b', function(rq,rs,n) {console.log('USE Func2');rs.send(rq.path);})
	.use('/greg/*', function(rq,rs,n) {console.log('USE Func3');rs.send(rq.path);})
	...
	function(rq,rs,n) {
		...
		fs.readFile(filename, function (err, data) {
			...
			console.log('USE Func4 ','content-type: ', rs.get('content-type'), 'content-length: ',rs.get('content-length'), 'data: ', data.toString())
			rs.send(data.toString());
		});
	})
	.use('/cookies/*', function(rq,rs,n) {console.log('USE Func5');rs.send(rq.cookies);})
	// params after?
	// cookies
	.start(8080, function(err){console.log(err);});
...
setTimeout(function() {
    http.request({host: 'localhost', port: 8080,  path: '/add/2/3/c/4'}, function(response) {console.log('Req Func2');response.on('data', function(data){console.log(data.toString());});}).end();
    }, 200);
